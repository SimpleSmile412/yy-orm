...
SimpleCond.prototype.transform = function(mapping) {
    ...
    ret.col = mapping[this.col] || this.col;
    return ret;
}

CombinedCond.prototype.transform = function(mapping) {
    var ret = _.clone(this);
    ret.conds = this.conds.map(cond => cond.transform(mapping));
    return ret;
}

DecorateCond.prototype.transform = function(mapping) {
    var ret = _.clone(this);
    ret.cond = this.cond.transform(mapping);
    return ret;
}
...
function limit(c, n, off) {
    var c_ = parse(_(arguments).filter(condFilter).nth(0) || cond.empty());
