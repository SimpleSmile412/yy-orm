...
DB.prototype.select = function(table, col, c, tx) {
    var tx_ = _(arguments).filter(item => item instanceof Transaction).nth(0);
    var col_ = _(arguments).slice(1).filter(item => _.isString(item) || _.isArray(item)).nth(0);
    var c_ = _(arguments).filter(item => item instanceof condType.Cond || _.isPlainObject(item)).nth(0);
    if (col_ === undefined) {
        col_ = "*";
    } else if (typeof col_ !== "string") {
        col_ = mysql.format("??", [col_]);
    }
    c_ = cond.parse(c_);
    var that = this;
    if (c_) {
        var condStr = c_.toSql();
        var sql = util.format("SELECT %s FROM %s WHERE %s", col_, table, condStr);
    } else {
