...
function DB(opt) {
    var dft = { supportBigNumbers: true, bigNumberStrings: true };
    this.pool = mysql.createPool(_.merge(opt, dft));
    this.models = {};
    this.$counter = 0;
}
module.exports = DB;
    ...
    return new Promise(function(resolve, reject) {
        that.pool.getConnection(function(err, conn) {
            if (err) {
                ...
                conn.$id = conn.$id || (conn.$id = ++that.$counter);
...
//'string' {object} {object}/'string'/Cond tx
DB.prototype.update = function(table, obj, c, tx) {
    assert(arguments.length >= 2);
    var tx_ = _(arguments).filter(txFilter).nth(0);
    var c_ = _(arguments).slice(2).filter(condFilter).nth(0);
    c_ = cond.parse(c_);
    if (c_ === undefined) {
        var fmt = "UPDATE ?? SET ?";
    } else {
